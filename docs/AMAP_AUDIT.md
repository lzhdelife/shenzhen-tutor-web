...
当前管理端展示平台侧实际发出的高德请求计数：当天总量、当月 POI 搜索量、当月基础 LBS 量、限流/超额次数，以及按接口和结果拆分的数据。REST 请求由 `cloudflare/amap-service.js` 上报，JS API 同源代理请求由 `/_AMapService` 上报，最终聚合在 D1 `amap_usage` 表中。

平台计数用于观察趋势、定位异常和提前预警，不是高德官方账单：缓存命中的查询不会产生上游请求，也不会计数；浏览器或高德 SDK 若存在未经过本站代理的资源请求，平台可能无法覆盖；高德对接口类别和扣量口径的调整也不会自动改变本站分类。真实额度、QPS 和超限状态始终以高德开放平台控制台为准。

服务端当前最多并发 4 个高德 REST 请求，并合并相同在途请求；地点候选缓存 5 分钟、地理编码缓存 30 分钟、路线缓存 2 分钟。订单导入先去重，再只为新增且地点未确认的订单调用 POI，避免重复订单消耗额度。早期约 200 名老师应先监控单个已认证账号的真实用量，规模增长后采用企业认证和正式容量方案，不通过多个个人账号轮换规避配额。
