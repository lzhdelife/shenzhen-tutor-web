...
- 老师：只收到公开订单字段。
- 发单者：与老师相同，复用精简公开订单读取；自己的完整订单信息在需要管理或读取联系方式时由专用接口按需获取。
- 管理员：可查看全部订单和质量异常信息。
...
公共订单查询在 D1 中直接限制创建时间晚于最近一次北京时间 6:00，并设置 2000 条当日订单安全上限。公开响应使用列表所需字段的精简投影，不包含完整 `structured_json`；管理员状态仍返回完整解析快照用于复核。
