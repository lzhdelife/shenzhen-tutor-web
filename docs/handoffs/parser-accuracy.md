...
- 混贴内容支持带说明前缀的 `#P...` 编号边界，不再把后续多条订单并进首条。
- `【报价】120-130/h` 不再被误判为“老师自报价”；价格区间保留最小值、最大值和代表值。
- 支持“400-600/两小时”“课时费500二小时”和“22000一个月”等常见计价写法，并仅在时长可靠时折算参考时薪。
- 地址行优先保留行政区、街道后的最终小区/花园，排除“日期+深圳机构”等来源抬头污染。
- 支持“暑假在 A，开学后在 B”的 `PHASED` 分阶段地点；括号中的“初中部/高中部”不再污染学生年级。
- 所有新增回归均为脱敏合成样本；真实反馈原文和联系方式未写入仓库。
