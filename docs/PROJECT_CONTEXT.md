...
1. 用户首次进入发单页提交称呼和微信号/手机号，并联系吴老师；申请与浏览器匿名发单身份绑定。
2. 管理员在“发单审核”批准后，后端才允许该身份解析、导入和发布订单。
3. 已批准用户在发单页粘贴一条或多条订单，或拖入 TXT。
4. 前端立即清空输入框并加入顺序队列，向用户显示排队、识别和发布状态。
5. 共享解析器负责切割、过滤非订单文本和结构化识别；UI、EXE 和地图模块不得复制解析逻辑。
6. 后端对未确认地点调用高德地点候选，保存结构化订单和地点结果。
7. 导入层使用原文规范化、订单编号和语义指纹阻止重复订单。
8. 开放订单在三天后按上海时区定时清理。
