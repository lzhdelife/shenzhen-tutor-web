...
- 普通用户看单打开即用，不要求注册或登录；浏览器生成匿名设备身份并在本地保存偏好。发单需提交称呼和微信号/手机号，经管理员审核批准。
...
1. 用户首次进入发单页提交称呼和微信号/手机号，并联系吴老师；审核记录归属于这组称呼和联系方式。
2. 管理员在“发单审核”批准后，后端才允许该身份解析、导入和发布订单。
3. 已批准用户可在其他浏览器填写相同称呼和联系方式，恢复同一个发单身份及其订单；不会生成重复申请。
4. 已批准用户在发单页粘贴一条或多条订单，或拖入 TXT。
5. 前端立即清空输入框并加入顺序队列，向用户显示排队、识别和发布状态。
6. 共享解析器负责切割、过滤非订单文本和结构化识别；UI、EXE 和地图模块不得复制解析逻辑。
7. 后端对未确认地点调用高德地点候选，保存结构化订单和地点结果。
8. 导入层使用原文规范化、订单编号和语义指纹阻止重复订单。
9. 开放订单在三天后按上海时区定时清理。
...
- 发单申请存储称呼、用户主动填写的微信号/手机号、审核状态和时间；不向普通看单用户公开。
- 管理员凭证经过客户端派生和服务端 Pepper 保护；真实密钥只保存在 Cloudflare Worker Secret 或本地 `.dev.vars`。
- 禁止把真实密钥、Token、私人手机号、订单数据或导出 SQL 提交到 Git；页面明确要求公开展示的平台联系人除外。
...
- 保留：发单人工审核、粘贴/TXT 导入、去重、三天清理、列表/地图、地点候选、直线距离、核心管理操作。
